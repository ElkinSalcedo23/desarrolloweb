...
public function Listar(){
   		try
		{
			...
			foreach($stm->fetchAll(PDO::FETCH_OBJ) as $r)
			{
				...
				$alm->__SET('ClaveUsuario', $r->passwd);
...
public function Listarmusico(){
   		try
		{
			$result = array();
			$stm = $this->pdo->prepare("SELECT * FROM usuario WHERE roles='Musico'");
			$stm->execute();
			foreach($stm->fetchAll(PDO::FETCH_OBJ) as $r)
			{
				$alm = new Usuario();
...
public function Listaragrupacion(){
		...
		{
			$result = array();
			$stm = $this->pdo->prepare("SELECT * FROM usuario WHERE roles='Agrupacion'");
			$stm->execute();
			foreach($stm->fetchAll(PDO::FETCH_OBJ) as $r)
			{
				$alm = new Usuario();
...
public function Existe($NombreUsuario){
	header("Content-Type: text/html;charset=utf-8");
       try{
        $estado='Habilitado';
		$stm = $this->pdo->prepare("SELECT * FROM usuario u WHERE 
				                         u.correo= ?");
...
public function Adicionar(usuario $data)
{
	try 
	{
